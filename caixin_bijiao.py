...
def main():
    ...
    with requests.session() as s:
        ...
        dir_path = "bijiao"
        new_magazine = update(s, "https://bijiao.caixin.com/", dir_path, "比较")
        # new_magazine = "https://bijiao.caixin.com/2025/cs_136/"
...
def update(session: requests.Session, magazine_url, index_file_dir, magazine_title):
    ...
    for i in del_dom:
        i.decompose()
    lis = wangqi.find("div", class_="wqCon").find_all("dt")
    for i in lis:
        a = i.find("a")
    ...
    img["src"] = urllib.request.pathname2url(
        os.path.join(href, "cover" + path[path.find("."):]))
    for i in ["lf", "ri"]:
