import logging, requests, os, urllib, time, traceback, json, datetime
import caixin
...
def main():
    ...
    with requests.session() as s:
        ...
        for topic in topics:
            ...
            logging.debug("topic: {}, topic_articles: {}".format(topic, topic_articles))
            topic_html = ""
            for i in topic_articles:
                logging.debug("{}".format(i))
                article_path = urllib.request.pathname2url(os.path.join(i.dir_path, "{}.html".format(i.article_id)))
                date_object = datetime.datetime.fromtimestamp(i.time)
                date = "{}-{:02d}-{:02d}".format(date_object.year, date_object.month, date_object.day)
                urlparse = urllib.parse.urlparse(i.pics)
                basename = "{}_{}".format(i.article_id, os.path.basename(urlparse.path))
                basename = basename.strip()
                img_path = urllib.request.pathname2url(os.path.join(i.dir_path, basename))
                caixin.download_img(i.pics, os.path.join(basedir, img_path), session = s)
                topic_html += topic_html_template.format(article_path, i.title, date, img_path)
            ...
            with open("{}/{}.html".format(basedir, topic["topic_id"]), "w", encoding = "utf-8") as f:
                f.write(caixin.template.format(title = topic["topic_title"], content = topic_html))
'''
    ...
    try:
        ...
        for i in download_articles:
            caixin.download_article(driver, i, os.path.join(basedir, i.dir_path))
...
def get_articles(session:requests.Session, topic) -> list[topic_article]:
    ...
    pageNum = 1
    while True:
        url = "https://entities.caixin.com/apientry/newTopic/getNewsTabContent?tabId={}&pageNum={}&pageSize=20".format(topic["topic_id"], pageNum)
        rsp = session.get(url).json()
        assert rsp["success"] == True, "get_articles fail, topic: {}, rsp: {}".format(topic, rsp)
    ...
    for i in articles:
        ...
        href = urllib.parse.urlunparse((parsed_url.scheme, parsed_url.netloc, parsed_url.path, '', '', ''))
        date_object = datetime.datetime.fromtimestamp(i["time"])
        path = "{}-{:02d}".format(date_object.year, date_object.month)
        ret.append(topic_article(i["id"], i["title"], href, i["time"], i["pics"], path))
